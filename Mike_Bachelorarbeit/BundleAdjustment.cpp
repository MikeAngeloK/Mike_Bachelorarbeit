...
void BundleAdjustment::adjustBundle(std::vector<WorldPoint3D>& pointCloud,std::vector<cv::Matx34d>& cameraPoses, Intrinsics& intrinsics, const std::vector<Features>& image2dFeatures) {
    ...
    for(int i = 0; i < pointCloud.size(); i++) {
        const WorldPoint3D& p = pointCloud[i];
        points3d[i] = cv::Vec3d(p.pt.x, p.pt.y, p.pt.z);

        for(const std::pair< int,int>& kv : p.views) {
            //kv.first  = camera index
            //kv.second = 2d feature index

            cv::Point2d p2d = image2dFeatures[kv.first].keypoints[kv.second].pt;
